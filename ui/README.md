...
Now you can rename `src/App.css` to `src/App.scss` and run `yarn watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/App.css`. Since `src/App.js` still imports `src/App.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/App.css` will be regenerated.
...
As a final step, you may find it convenient to run `watch-css` automatically with `yarn start`, and run `build-css` as a part of `yarn build`. You can use the `&&` operator to execute two scripts sequentially. However, there is no cross-platform way to run two scripts in parallel, so we will install a package for this:
...
Now running `yarn start` and `yarn build` also builds Sass files.
...
Install the Firebase CLI if you haven’t already by running `yarn add -g firebase-tools`. Sign up for a [Firebase account](https://console.firebase.google.com/) and create a new project. Run `firebase login` and login with your previous created Firebase account.
...
1. Install the `now` command-line tool either via the recommended [desktop tool](https://zeit.co/download) or via node with `yarn add -g now`.
...
Install the Surge CLI if you haven’t already by running `yarn add -g surge`. Run the `surge` command and log in you or create a new account.
